...
import (
	...
	"github.com/Wafer233/WaferTakeOut/Server/wafer-take-out-server/employee-service/internal/domain"
	"github.com/Wafer233/WaferTakeOut/Server/wafer-take-out-server/employee-service/pkg/jwt"
...
func (svc *EmployeeService) Login(ctx context.Context, dto *LoginDTO) (*LoginVO, error) {
	...
	if entity.Status == 0 {
		return nil, errors.New("已被禁用")
	}

	token, err := jwt.GenerateToken(entity.Id)
